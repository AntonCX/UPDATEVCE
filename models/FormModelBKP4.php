...
//===================================================================================================================
        $queryP = "UPDATE PAR_FACTV03 SET PREC = ?,TOT_PARTIDA =? WHERE NUM_PAR = 1 AND CVE_DOC = ?";
        $paramsP = array($newValue, $newValue, $cve_doc);
	...
	public function insertCuenM($cve_doc, $campo4) {
        ...
        if ($rowCheck['count'] > 0) {
            ...
            $queryInsert = "INSERT INTO CUEN_M03 (CVE_CLIE, REFER, NUM_CPTO, NUM_CARGO, CVE_OBS, NO_FACTURA, DOCTO, IMPORTE,
                ...
                VALUES (?, ?, 2, 1, 0, ?, ?, ?, GETDATE(), GETDATE(), 'A', ' ', 1, 1, ?, GETDATE(), 'C', 1, ' ', 'A', NEWID(), GETDATE(), ' ')";
